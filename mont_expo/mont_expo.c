...
*/
void montgomery_exp(unsigned long long p1[], unsigned long long p2[], unsigned long long r[], int size, int bit_limit, mpz_t mod){
    ...
    for(ssize_t i = bits-1;i>=0;i--){
        unsigned long long temp1[size];
        unsigned long long temp2[size];
        for(int i =0;i<size;i++){
            temp1[i] = 0;
            temp2[i] = 0;
        }
        mpz_export(temp1, NULL, 1, sizeof(unsigned long long), 0, 0, mont_res);
        montgomery_pr(temp1, temp1, temp2, size, bit_limit, mod);
        mpz_import(mont_res, size, 1, sizeof(unsigned long long), 0, 0, temp2);

        if(mpz_tstbit(expo, i)){
            unsigned long long mont_res_mul_arr[size];
            unsigned long long mont_base_arr[size];
            unsigned long long mul_result[size];
            for(int i =0;i<size;i++){
                mont_res_mul_arr[i] = 0;
                mont_base_arr[i] = 0;
                mul_result[i] = 0;
            }
            ...
            montgomery_pr(mont_res_mul_arr, mont_base_arr, mul_result, size, bit_limit, mod);
