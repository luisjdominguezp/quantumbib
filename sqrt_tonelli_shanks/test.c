#include <stdio.h>
#include <gmp.h>
#include <inttypes.h>
#include <x86intrin.h>

#define SIZE 4
#define P_SIZE 5
#define R_SIZE P_SIZE
#pragma intrinsic(__rdtsc)
#define NTEST 100000

void measured_function(volatile int *var) { (*var) = 1; }

void t_sqrt(unsigned long long p1[], unsigned long long p2[], unsigned long long r[]) {
    mpz_t p, n, r_mpz, q, s, z, c, t, m, b, temp, exponent;
    unsigned long e;
    int found = 0;

    mpz_inits(p, n, r_mpz, q, s, z, c, t, m, b, temp, exponent, NULL);

    // Import array representations to mpz_t using correct sizes
    mpz_import(n, SIZE, 1, sizeof(unsigned long long), 0, 0, p1);
    mpz_import(p, P_SIZE, 1, sizeof(unsigned long long), 0, 0, p2);

    gmp_printf("Value of p1: %Zd\n", n);
    gmp_printf("Value of p2: %ZX\n", p);
    // Check if n is a quadratic residue modulo p
    if (mpz_legendre(n, p) != 1) {
        gmp_printf("No square root exists.\n");
        mpz_clears(p, n, r_mpz, q, s, z, c, t, m, b, temp, exponent, NULL);
        return;
    }

    // Step 1: Find Q and S such that p - 1 = Q * 2^S with Q odd
    mpz_sub_ui(q, p, 1);  // q = p - 1
    mpz_set_ui(s, 0);
    while (mpz_even_p(q)) {
        mpz_fdiv_q_2exp(q, q, 1);  // q = q / 2
        mpz_add_ui(s, s, 1);       // s = s + 1
    }

    // Step 2: Find a quadratic non-residue z modulo p
    mpz_set_ui(z, 2);
    while (!found) {
        if (mpz_legendre(z, p) == -1) {
            found = 1;
        } else {
            mpz_add_ui(z, z, 1);
        }
    }

    // Step 3: Initialize variables
    mpz_powm(c, z, q, p);

    // Compute exponent = (Q + 1) / 2
    mpz_add_ui(temp, q, 1);                   // temp = Q + 1
    mpz_fdiv_q_2exp(exponent, temp, 1);       // exponent = (Q + 1) / 2

    // Compute r_mpz = n^{(Q + 1)/2} mod p
    mpz_powm(r_mpz, n, exponent, p);

    // Compute t = n^Q mod p
    mpz_powm(t, n, q, p);

    mpz_set(m, s);

    // Step 4: Main loop until t == 1
    while (mpz_cmp_ui(t, 1) != 0) {
        // Find the smallest integer e such that t^(2^e) ≡ 1 mod p
        mpz_set(temp, t);
        e = 0;
        while (mpz_cmp_ui(temp, 1) != 0 && e < mpz_get_ui(m)) {
            mpz_powm_ui(temp, temp, 2, p);
            e++;
        }
        if (e >= mpz_get_ui(m)) {
            gmp_printf("No square root exists.\n");
            mpz_clears(p, n, r_mpz, q, s, z, c, t, m, b, temp, exponent, NULL);
            return;
        }

        // Compute b = c^{2^{m - e - 1}} mod p
        mpz_sub_ui(temp, m, e + 1);                  // temp = m - e - 1
        mpz_ui_pow_ui(temp, 2, mpz_get_ui(temp));    // temp = 2^{m - e - 1}
        mpz_powm(b, c, temp, p);

        // Update r = r * b mod p
        mpz_mul(r_mpz, r_mpz, b);
        mpz_mod(r_mpz, r_mpz, p);

        // Update t = t * b^2 mod p
        mpz_powm_ui(b, b, 2, p);  // b = b^2 mod p
        mpz_mul(t, t, b);
        mpz_mod(t, t, p);

        // Update c = b
        mpz_set(c, b);

        // Update m = e
        mpz_set_ui(m, e);
    }

    // Export the result back to array representation
    size_t count;
    mpz_export(r, &count, -1, sizeof(unsigned long long), 0, 0, r_mpz);
    mpz_clears(p, n, r_mpz, q, s, z, c, t, m, b, temp, exponent, NULL);
}

int main() {
    uint64_t start, end;
    int variable = 0;

    unsigned long long p1[SIZE] = {0, 0, 0, 4}; // Example input N
    unsigned long long p2[P_SIZE] = {
        0xA7081AEA3BDBF56E,
        0x0AE5736BE1124F8D,
        0xC7CE6E75FAC521DD,
        0x9F6A6B593208CDF6,
        0x0E83615E354157D9
    };
    /*
    0x0E83615E354157D9,
0x9F6A6B593208CDF6,
0xC7CE6E75FAC521DD,
0x0AE5736BE1124F8D,
0xA7081AEA3BDBF56E
    */
    unsigned long long result[R_SIZE] = {0};

    printf("Warming up the cpu.\n");
    for (int i = 0; i < NTEST; i++) {
        measured_function(&variable);
    }

    printf("Calculating Result...\n");
    start = __rdtsc();
    t_sqrt(p1, p2, result);
    end = __rdtsc();

    // Check if result is non-zero
    int has_result = 0;
    for (int i = 0; i < R_SIZE; i++) {
        if (result[i] != 0) {
            has_result = 1;
            break;
        }
    }

    if (!has_result) {
        printf("No square root exists.\n");
    } else {
        printf("Square root modulo p is:\n");
        for (int i = R_SIZE - 1; i >= 0; i--) {
            printf("%016llX\n", result[i]);
        }
    }

    printf("Total = %f CPU cycles\n", (float)(end - start) / NTEST);

    // Verification
    mpz_t p_mpz, r_mpz, n_mpz, temp;
    mpz_inits(p_mpz, r_mpz, n_mpz, temp, NULL);

    mpz_import(n_mpz, SIZE, 1, sizeof(unsigned long long), 0, 0, p1);
    mpz_import(p_mpz, P_SIZE, 1, sizeof(unsigned long long), 0, 0, p2);
    mpz_import(r_mpz, R_SIZE, -1, sizeof(unsigned long long), 0, 0, result);

    gmp_printf("Value of N: %Zd\n", n_mpz);
    gmp_printf("Value of p: %ZX\n", p_mpz);
    gmp_printf("Value of res or g: %ZX\n", r_mpz);
    mpz_powm_ui(temp, r_mpz, 2, p_mpz); // temp = r^2 mod p

    if (mpz_cmp(temp, n_mpz) == 0) {
        printf("Verification succeeded: r^2 ≡ n mod p\n");
    } else {
        printf("Verification failed: r^2 does not equal n mod p\n");
    }

    mpz_clears(p_mpz, r_mpz, n_mpz, temp, NULL);

    return 0;
}

